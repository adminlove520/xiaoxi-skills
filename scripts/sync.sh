...
if [ -d "$OPENCLAW_SKILLS" ]; then
    ...
    find "$OPENCLAW_SKILLS" -maxdepth 2 -name "SKILL.md" -exec dirname {} \; | while read -r skill_path; do
        ...
        if [ ! -d "$REPO_DIR/workspace/$skill_name" ]; then
...
echo "📂 仓库目录: $REPO_DIR"
echo "📊 当前仓库 Skill 统计:"
echo "  - Workspace: $(ls -1 "$REPO_DIR/workspace" 2>/dev/null | wc -l) 个"
echo "  - OpenClaw: $(ls -1 "$REPO_DIR/openclaw" 2>/dev/null | wc -l) 个"
echo "  - Agents: $(ls -1 "$REPO_DIR/agents" 2>/dev/null | wc -l) 个"
echo ""
echo "下一步:"
echo "  cd $REPO_DIR"
echo "  git add ."
