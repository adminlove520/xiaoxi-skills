...
Minimal implementation for binding operations.
Only used if numpy is available; otherwise the fact store works without HRR.
"""
...
def encode_fact(content: str, entities: list[str], dim: int = 1024) -> "np.ndarray":
    """Structured encoding: content bound to ROLE_CONTENT, each entity bound to ROLE_ENTITY."""
    _require_numpy()
