...
const INSTALL_COMMANDS = {
  ...
  'chrome-cdp': { install: 'git clone https://github.com/pasky/chrome-cdp-skill.git ~/.openclaw/skills/chrome-cdp', url: 'https://github.com/pasky/chrome-cdp-skill' },
  'myclaw-backup': { install: 'git clone https://github.com/myclawai/myclaw-backup.git ~/.openclaw/skills/myclaw-backup', url: 'https://github.com/myclawai/myclaw-backup' },
  ...
  'summarize': { install: 'npx summarize-cli', url: 'https://www.npmjs.com/package/summarize-cli' },
};

function getInstallInfo(name) {
  if (INSTALL_COMMANDS[name]) return INSTALL_COMMANDS[name];
  return {
...
export async function GET(request) {
  const localSkills = LOCAL_SKILLS.map(s => {
    const info = getInstallInfo(s.name);
    return { ...s, source: 'local', status: 'ready', install: info.install, url: info.url };
  });

  const builtinSkills = BUILTIN_SKILLS.map(s => ({
    ...s, source: 'openclaw', status: 'builtin', install: '内置技能', url: null
  }));

  const pendingSkills = PENDING_SKILLS.map(s => ({
    ...s, source: 'pending', status: 'pending', install: '待发布', url: null
  }));

  const allSkills = [...localSkills, ...builtinSkills, ...pendingSkills];
