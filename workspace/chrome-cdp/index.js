 ...
 * 
 * 安装: 
 *   1. git clone https://github.com/pasky/chrome-cdp-skill.git
 *   2. 将 scripts 目录复制到此目录
 */
...
const SCRIPT_URL = 'https://raw.githubusercontent.com/pasky/chrome-cdp-skill/main/scripts/cdp.mjs';

const SCRIPT_PATH = path.join(__dirname, 'scripts', 'cdp.mjs');
...
function ensureScript() {
  ...
  if (!fs.existsSync(SCRIPT_PATH)) {
    console.log('⚠️  正在下载 cdp.mjs...');
    try {
      const https = require('https');
      const http = require('http');
      
      const file = fs.createWriteStream(SCRIPT_PATH);
      
      return new Promise((resolve, reject) => {
        const protocol = SCRIPT_URL.startsWith('https') ? https : http;
        protocol.get(SCRIPT_URL, (response) => {
          response.pipe(file);
          file.on('finish', () => {
            file.close();
            console.log('✅ cdp.mjs 下载完成');
            resolve(true);
          });
        }).on('error', (err) => {
...
function cdp(command, args = []) {
  const ready = ensureScript();
  if (!ready) {
    return { error: 'CDP脚本未安装，请先运行: git clone ' + CDP_REPO };
  }
  
  try {
    const cmd = `node "${SCRIPT_PATH}" ${command} ${args.join(' ')}`;
