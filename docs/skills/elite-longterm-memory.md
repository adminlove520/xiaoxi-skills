...
name: elite-longterm-memory version: 1.2.3 description: "Ultimate AI agent memory system for Cursor, Claude, ChatGPT & Copilot. WAL protocol + vector search + git-notes + cloud backup. Never lose context again. Vibe-coding ready." 

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/elite-longterm-memory ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/elite-longterm-memory/SKILL.md
```
