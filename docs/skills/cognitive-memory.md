...
name: cognitive-memory description: Intelligent multi-store memory system with human-like encoding, consolidation, decay, and recall. Use when setting up agent memory, configuring remember/forget triggers, enabling sleep-time reflection, building knowledge graphs, or adding audit trails. Replaces ba

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/cognitive-memory ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/cognitive-memory/SKILL.md
```
