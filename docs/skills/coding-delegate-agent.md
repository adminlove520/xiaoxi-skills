...
name: coding-agent description: 'Delegate coding tasks to Codex, Claude Code, or Pi agents via background process. Use when: (1) building/creating new features or apps, (2) reviewing PRs (spawn in temp dir), (3) refactoring large codebases, (4) iterative coding that needs file exploration. NOT for: 

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/coding-delegate-agent ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/coding-delegate-agent/SKILL.md
```
