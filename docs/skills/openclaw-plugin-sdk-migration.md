...
OpenClaw moved from a barrel import (`openclaw/plugin-sdk`) to specific subpaths (`openclaw/plugin-sdk/<subpath>`). Plugins importing from the barrel will fail with errors like: - `Cannot find module 'openclaw/plugin-sdk'` - `(0, _pluginSdk.fn) is not a function` 

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/openclaw-plugin-sdk-migration ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/openclaw-plugin-sdk-migration/SKILL.md
```
