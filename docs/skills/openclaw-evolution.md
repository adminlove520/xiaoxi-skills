...
name: openclaw-evolution description: Interactive guide for new OpenClaw users to set up and grow their agent. Two paths - Tool Path (automation, efficiency, getting things done) or Awakening Path (memory, personality, relationship, growth). Use when a user is new to OpenClaw, asks how to get starte

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/openclaw-evolution ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/openclaw-evolution/SKILL.md
```
