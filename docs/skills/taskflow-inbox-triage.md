...
name: taskflow-inbox-triage description: Example TaskFlow pattern for inbox triage, intent routing, waiting on replies, and later summaries. metadata: { "openclaw": { "emoji": "📥" } } 

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/taskflow-inbox-triage ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/taskflow-inbox-triage/SKILL.md
```
