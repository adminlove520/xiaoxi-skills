...
name: openclaw-pr-maintainer description: Review, triage, close, label, comment on, or land OpenClaw PRs/issues with maintainer evidence checks. Use this skill for maintainer-facing GitHub workflow, not for ordinary code changes. 

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/openclaw-pr-maintainer ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/openclaw-pr-maintainer/SKILL.md
```
