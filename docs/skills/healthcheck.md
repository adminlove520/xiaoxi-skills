...
name: healthcheck description: Audit and harden hosts running OpenClaw for SSH, firewall, updates, exposure, cron checks, and risk posture. Assess and harden the host running OpenClaw, then align it to a user-defined risk tolerance without breaking access. Use OpenClaw security tooling as a first-cl

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/healthcheck ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/healthcheck/SKILL.md
```
