...
name: memory-curator description: Manage agent memory through daily logs, session preservation, and knowledge extraction. Use when (1) logging work at end of day, (2) preserving context before /new or /reset, (3) extracting patterns from daily logs to MEMORY.md, (4) searching past decisions and lea

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/memory-curator ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/memory-curator/SKILL.md
```
