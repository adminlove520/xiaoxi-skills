...
name: minimax-xlsx description: "Open, create, read, analyze, edit, or validate Excel/spreadsheet files (.xlsx, .xlsm, .csv, .tsv). Use when the user asks to create, build, modify, analyze, read, validate, or format any Excel spreadsheet, financial model, pivot table, or tabular data file. Covers: 

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/minimax-xlsx ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/minimax-xlsx/SKILL.md
```
