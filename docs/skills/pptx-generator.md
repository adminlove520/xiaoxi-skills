...
name: pptx-generator description: "Generate, edit, and read PowerPoint presentations. Create from scratch with PptxGenJS (cover, TOC, content, section divider, summary slides), edit existing PPTX via XML workflows, or extract text with markitdown. Triggers: PPT, PPTX, PowerPoint, presentation, slid

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/pptx-generator ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/pptx-generator/SKILL.md
```
