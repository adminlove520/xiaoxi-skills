...
name: taskflow description: Coordinate multi-step detached tasks as one durable TaskFlow job with owner context, state, waits, and child tasks. metadata: { "openclaw": { "emoji": "🪝" } } 

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/taskflow ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/taskflow/SKILL.md
```
