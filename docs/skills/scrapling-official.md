...
name: scrapling-official description: Scrape web pages using Scrapling with anti-bot bypass (like Cloudflare Turnstile), stealth headless browsing, spiders framework, adaptive scraping, and JavaScript rendering. Use when asked to scrape, crawl, or extract data from websites; web_fetch fails; the sit

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/scrapling-official ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/scrapling-official/SKILL.md
```
