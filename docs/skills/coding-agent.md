...
name: coding-agent description: 'Delegate coding tasks to Codex, Claude Code, OpenCode, or Pi agents via immediate background processes. Use when: (1) building or creating features/apps, (2) reviewing PRs in a temp clone/worktree, (3) refactoring large codebases, (4) iterative coding that needs file

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/coding-agent ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/coding-agent/SKILL.md
```
