...
name: stealth-browser description: Ultimate stealth browser automation with anti-detection, Cloudflare bypass, CAPTCHA solving, persistent sessions, and silent operation. Use for any web automation requiring bot detection evasion, login persistence, headless browsing, or bypassing security measures.

## 安装

```bash
cp -r ~/.openclaw/xiaoxi-skills/stealth-browser ~/.openclaw/skills/
```
...
```bash
# 查看详情
cat ~/.openclaw/skills/stealth-browser/SKILL.md
```
